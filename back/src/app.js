...
io.on("connection", socket => {
        ...
        var parsed = fs.readFile('./data.json', 'utf8', (err, jsonString) => {
            ...
            for( let prop in parsed ){
                ...
                for (let message in parsed[prop]) {
                    io.to(socket.id).emit('new-message', [parsed[prop][message].by, parsed[prop][message].date , parsed[prop][message].message, prop, color]);
                }
            }
    ...
    socket.on("leave", data => {
        ...
        io.to(socket.id).emit("new-message", [null,null,"Vous avez quitté le salon " + data.id, "info", "#000000"]);
    });
    ...
    socket.on('deleteDoc', function(channel){
        ...
        io.in(channel).clients(function(error, clients) {
            ...
            io.to("general").emit('new-message', [null,null,msg.content, msg.channel, "#FFFFFF"]);
        });
    ...
    socket.on('new-message', (message) => {
        ...
        if(command === "/mp") {
            ...
            if(getKey(people, message.message.split(" ")[1]) === socket.id) {
                io.to(socket.id).emit('new-message', [null,null,"Vous ne pouvez pas vous envoyer des messages à vous même", "info", "#000000"]);
                return;
            }
        ...
        if(message.message[0] === "/" && !documents[command.substring(1)]) {
            io.to(socket.id).emit('new-message', [null,null,"Commande ou salon introuvable", "info", "#000000"]);
            return;
        }
        ...
        io.to(message.id).emit('new-message', [people[socket.id], today, message.message, message.id, colors[message.id]]);
